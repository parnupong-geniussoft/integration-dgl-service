...
func (data *LoggerStruct) GetReferenceId(ctx *fiber.Ctx) {
	...
	if ctx.Path() == "/v1/integration-api/dgl/add_dgl" {
		referenceId, err = getReferenceIdFromBodyForAddDgl(ctx.Body())
		if err != nil {
			return
		}
	} else {
...
func getReferenceIdFromBodyForAddDgl(x []byte) (*string, error) {
	var bodyMap map[string]interface{}
	var referenceId string
